...
export default function SessionsListClient({ sessions }: Props) {
  ...
  useEffect(() => {
    ...
    // on utilise le slug comme identifiant logique
    sessions.forEach((session) => {
      map[session.slug] = {
        doneToday: getSessionDoneToday(session.slug),
        lastEntry: getLastSessionEntry(session.slug),
      };
    });
    setStatusMap(map);

    // 1) valeur initiale depuis localStorage
    const localDone = getSessionsDoneThisWeek();
    const localKnown = localDone.filter((slug) =>
      sessions.some((s) => s.slug === slug)
    );
    setDoneThisWeekCount(localKnown.length);

    // 2) puis on remplace par la version BDD si dispo
    void (async () => {
      try {
        ...
        const dbKnown = slugs.filter((slug: string) =>
          sessions.some((s) => s.slug === slug)
        );
        setDoneThisWeekCount(dbKnown.length);
      } catch (e) {
        console.error("[Sessions] Erreur stats DB:", e);
      }
    })();
  }, [sessions]);
    ...
    <main className="px-4 pb-4 space-y-5">
      ...
      <section className="space-y-3">
        {sessions.map((session) => {
          const status = statusMap[session.slug];
